...
public class LecturerAttendanceController {
    ...
    public void initialize() {
        colStudentId.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().get(0)));
        colStudentName.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().get(1)));
        colRegNo.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().get(2)));
        colStatus.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().get(3)));
        ...
        colStatus.setOnEditCommit(event -> event.getRowValue().set(3, event.getNewValue()));
    ...
    private void loadSessions() {
        try {
            ...
            String sql = "SELECT cs.session_id, s.subject_name, cs.session_date " +
                         ...
                         "WHERE cs.lecturer_id = ? ORDER BY cs.session_date DESC";
